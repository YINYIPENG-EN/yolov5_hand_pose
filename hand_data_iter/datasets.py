...
def draw_bd_handpose(img_,hand_,x,y):
    ...
    # hand_是dict,包含了21点坐标信息，{'0':{'x': x坐标, 'y':y坐标}}
...
def plot_box(bbox, img, color=None, label=None, line_thickness=None):
    tl = line_thickness or round(0.002 * max(img.shape[0:2])) + 1  # 线宽
    color = color or [random.randint(0, 255) for _ in range(3)]  # 颜色
    c1, c2 = (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3]))
    cv2.rectangle(img, c1, c2, color, thickness=tl)# 绘制目标的bbox
    if label:
        tf = max(tl - 2, 1)
...
class LoadImagesAndLabels(Dataset):
    def __init__(self, ops, img_size=(224,224), flag_agu = False,fix_res = True,vis = False):
        ...
        path = ops.train_path  # 数据集路径

        file_list = []  # 文件列表
        hand_anno_list = []  # 手标签列表
        idx = 0
        for f_ in os.listdir(path):
            if ".jpg" in f_:  # 判断是否为jpg格式
                img_path = path +f_  # 获得图片路径
                label_path = img_path.replace('.jpg','.json')  # 获得label路径
                if not os.path.exists(label_path):
                    continue

                f = open(label_path, encoding='utf-8')  # 读取 json文件
                hand_dict_ = json.load(f)  # 读取json，dict形式
                f.close()
                if len(hand_dict_)==0:
                    continue

                hand_dict_ = hand_dict_["info"]  # 获得info字典的value值，'pts': {'0': {'x': 69, 'y': 120}, '1': {'x': 49, 'y': 113}
                ...
                if len(hand_dict_)>0:  # 读取成功
                    for msg in hand_dict_:  # 获取每个手的信息
                        bbox = msg["bbox"]  # 获得手的box框
                        pts = msg["pts"]  # 获得21关键点的类和坐标   {'0': {'x': 69, 'y': 120}, '1': {'x': 49, 'y': 113}
    ...
    def __len__(self):
        return len(self.files)  # 获得文件长度

    def __getitem__(self, index):
        img_path = self.files[index]  # 通过迭代获取图片路径
        bbox, pts = self.hand_anno_list[index]  # 获得手的box和 pts{包含了类和对应的坐标}
        img = cv2.imread(img_path)  # BGR
        #-------------------------------------
        x1,y1,x2,y2 = int(bbox[0]),int(bbox[1]),int(bbox[2]),int(bbox[3])  # box信息
        ...
        y_min = 65535
        for i in range(21):
            x_, y_ = pts[str(i)]["x"],pts[str(i)]["y"]  # pts[str(i)]是获得每个关键点。这行代码是获取每个关键点的坐标
            x_ += x1
            y_ += y1
            pts_.append([x_, y_])
            x_min = x_ if x_min > x_ else x_min
